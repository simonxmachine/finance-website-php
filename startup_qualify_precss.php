<?php
    ...
    if($_SERVER['REQUEST_METHOD']=="POST" && isset($_SESSION['token']) && isset($_POST['token']) && $_SESSION['token'] ==$_POST['token']){
        ...
        if ($Error ==""){
            ...
            $query = "INSERT INTO user_full(first_name, last_name, email, phone, city, state, zipcode, dob, credit, checkbox, llc, url_address, ip) VALUES(:first_name, :last_name, :email, :phone, :city, :state, :zipcode, :dob, :credit, :checkbox1, :llc, :url_address, :ip)";
            $stm = $connection->prepare($query);
            $stm -> execute($arr);

            $to = 'simonremgmt@gmail.com';
            $subj = 'NEW STARTUP LEAD';
            $msg = 'Name: ' . $first_name . '<br />' . 'Last Name: ' . $last_name . '<br />' . 'Phone: ' . $phone;
...
<body style="font-family: Poppins">
    ...
    <form method="POST">
        ...
        <input type="text" name='phone' value='<?=$phone?>' placeholder="Mobile Phone Number (ex. 777-777-7777)" class="form-control" minlength="10" maxlength="12" pattern="[0-9]{3}-[0-9]{3}-[0-9]{4}" required=""
        ...
        <select name="state" id="state" value='<?=$state?>' placeholder="State" class="form-control" required="" oninvalid="this.setCustomValidity('Please Select State')" oninput="setCustomValidity('')">
        ...
        <input type="text" name="zipcode" value='<?=$zipcode?>' placeholder="Postal Zip Code" class="form-control" required="" 
        ...
        <input type="text" name="dob" value='<?=$dob?>' placeholder="Birthdate (mm/dd/yyyy)" class="form-control" required="" 
        ...
        <input type='submit' name='send' value='Apply Now'>
